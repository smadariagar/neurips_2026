...
\usepackage{amsmath}
\usepackage{graphicx}
\input{macros/macroadmin}
% Note. For the workshop paper template, both \title{} and \workshoptitle{} are required, with the former indicating the paper title shown in the title and the latter indicating the workshop title displayed in the footnote. 
\title{Hierarchical Lateral Inhibition Enables Robust Winner-Take-All Computation in Large-Scale Cortical Models}
...
% \end{abstract}
% COMENZARIA CON UNA FRASE MUY GENERAL AL TEMA ALGO COMO: IN COMPUTATIONAL NEUROSCIENCE, MODELS OF THE CLASSICAL VISUAL RECEPTIVE FIELD HAS BEEN VASTLY STUDIED, BUT IT HAS REMAIN ELUSIVE TO MODEL THE EFFECTS OBSERVED WHEN STUDYING THE EXTRA-CLASSICAL VISUAL RECEPTIVE FIELD.
\begin{abstract}
   In computational neuroscience, models of visual cortex have long captured classical receptive field properties, yet the circuit mechanisms underlying extra-classical surround suppression remain unresolved. \removehide{Winner-take-all (WTA) dynamics, where a strongly stimulated cortical column suppresses neighboring activity by over 75\%, are consistently measured in vivo and are essential for functions like figure-ground segmentation.}In vivo experiments have consistently shown a winner-take-all (WTA) dynamic, where a strongly stimulated cortical column suppresses neighboring activity by over 75\%; this dynamic is essential for basic visual functions like figure-ground segmentation. Classical models attribute this computation to lateral inhibition within V1, yet prior work has struggled to reproduce the magnitude of suppression observed physiologically using models of V1 circuits alone.
   In this work, we hypothesized that robust WTA emergence requires extending lateral inhibitory competition to higher cortical areas, beyond V1. To test this hypothesis, we simulated a large-scale spiking neural network based on the connectivity of the canonical cortical microcircuit and systematically compared three different architectures: \removehide{To test this hypothesis, we systematically compared three architectures within a large-scale spiking neural network based on the connectivity of canonical cortical microcircuit:}(1) V1 columns with lateral inhibition restricted to V1, (2) a hierarchical V1-V2 model with simple feedback between V1 and V2 but no lateral inhibition in V2, and (3) a hierarchical model with feedback where both V1 and V2 incorporate lateral inhibitory connections. Our experiments show that only when lateral competition is extended to V2 do the circuits generate a physiologically accurate surround suppression effect, achieving a suppression index of ~75\% that quantitatively matches in vivo recordings. \removehide{Our simulations demonstrate that lateral inhibition confined to V1 is insufficient to replicate biological WTA dynamics.}In this architecture, V2 lateral inhibition gates the excitatory feedback returning to V1, enabling the network to dynamically encode relative stimulus strength even when absolute input remains constant.\removehide{This work establishes that lateral inhibition in higher cortical areas is a computationally necessary architectural principle for robust WTA normalization. Beyond explaining a long-standing physiological puzzle, this hierarchical inhibitory architecture provides a blueprint for designing artificial systems that require context-dependent, relative information processing.}
   Our work explains a long-standing physiological puzzle and establishes that lateral inhibition in higher cortical areas is necessary to drive the circuit from a regime of general divisive normalization into a robust winner-take-all (WTA) state, which is essential for strong surround suppression. Moreover, this hierarchical inhibitory architecture provides a blueprint for designing systems that require context-dependent information processing.   \removehide{\commentcd{Hasta este punto no se ha mencionado qué es "WTA normalization" creo que sería bueno agregar una frase al respceto al comienzo. Eso además permitiría al lector entender mejor porque es importante "encode relative stimulus strength even when absolute input remains constant"}\commentsm{Prefiero sacar lo de "WTA normalization", tampoco tengo mucho como justificarlo}}
\end{abstract}
