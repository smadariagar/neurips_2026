...
%
``The complete connection probability matrix between the eight neuronal populations (four layers $\times$ two types) follows the values reported by \citet{Potjans2014} and is provided in Table~\ref{tab:connectivity} of the Appendix for completeness.''
...
%
``To isolate the contribution of each circuit component, we systematically compared three architectural configurations (see Figure~\ref{fig:architectures}):
\begin{description}
  \item[Configuration 1 (V1 lateral only):] Four V1 microcolumns arranged in a ring, interconnected exclusively via horizontal connections within L2/3. No V2 modules are present.\footnote{Se puede poner esto como pie de figura en lugar de aquí.}
  \item[Configuration 2 (V1 + linear V2 feedback):] The V1 columns project feedforward to a \emph{single} V2 module, which pools inputs from all columns and returns non-specific feedback to V1. This configuration implements hierarchical feedback without lateral competition.
  \item[Configuration 3 (V1 + competitive V2 feedback):] The V1 columns project feedforward to \emph{two} V2 modules, each aligned to a specific orientation column. Critically, the two V2 modules are connected via strong, reciprocal inhibitory synapses, implementing a winner-take-all competition.
\end{description}
...
%
``A schematic of the three configurations is provided in Figure~\ref{fig:architectures}.''
\begin{figure}[t]
  \centering
