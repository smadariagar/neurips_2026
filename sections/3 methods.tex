...
%
The complete connection probability matrix between the eight neuronal populations (four layers $\times$ two types) follows the values reported by \citet{Potjans2014} and is provided in Table~\ref{tab:connectivity} of the Appendix.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{img/architectures.pdf}
    \caption{\textbf{Canonical Microcircuit and Hierarchical Architecture.} (Left) Local and intra-areal connectivity of the V1 microcircuit, detailing the specific interlaminar excitatory (E) and inhibitory (I) projections. (Right) Inter-areal feedforward and feedback connectivity between V1 and V2, along with near and distant lateral connections within V1.}
...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Stimulation Protocols and Network Configurations.} Schematic representation of the classical receptive field (CRF) and extra-classical receptive field (eCRF) paradigms across the three evaluated architectures. The bottom panels illustrate the thalamic stimulation over time and the corresponding peri-stimulus time histograms (PSTH) for Layer 2/3 neurons in V1, highlighting the varying degrees of surround suppression.}
    \label{fig:protocol}
\end{figure}
